/*
  ...
  Automated deserializing of classes from json object
*/
...
export class Dejsonizer
{
  ...
  // // ! Throws exception on error.
  // private plainObjectFromJson(json: object, target: object): object
  // {
  //   if (target === null || target === undefined)
  //     target = {};

  //   // ! Throws exception on error.
  //   this.objectFromJson(json, target);
  ...
  {
    const id = this.getStringProperty(json, ID);
  ...
  // ! Throws exception on error.
  private getStringProperty(json: object, propertyName: string): string
  {
    const property = this.getProperty(json, propertyName);
...
//     json: Types.Object,
//     target: Types.Object,
//     customPropertyFromJson?: CustomPropertyFromJsonFunction,
//     path?: string
//   )
//   : void
//   {
...
//       // ! Throws exception on error.
//       // We are cycling over properties in source JSON object, not in target
//       // object to ensure that properties which are not present in source
//       // data
//       // will not get overwritten with 'undefined'. This allows adding new
//       // properties to existing classes without the need to convert all save
//       // files.
//       const property = propertyFromJson
//       (
//         propertyName,
//         target[propertyName],
//         json[propertyName],
...
// {
//   // Allow custom property deserialization.
//   if (customPropertyFromJson)
//     return customPropertyFromJson(propertyName, json, target, path);

//   // ! Throws exception on error.
//   validateJsonAndTarget(propertyName, json, target, path);
...
// // ! Throws exception on error.
// function validateJsonAndTarget
// (
//   propertyName: string,
//   json: any,
//   target: any,
...
//   if (Array.isArray(json))
//     // ! Throws exception on error.
//     return arrayFromJson(json, customPropertyFromJson, path);

//   const className = readProperty(json, CLASS_NAME);

//   if (className === undefined)
//     return plainObjectFromJson(json, target, customPropertyFromJson, path);
...
//     case SET_CLASS_NAME:
//       // ! Throws exception on error.
//       return setFromJson(json, propertyName, path);

//     case MAP_CLASS_NAME:
//       // ! Throws exception on error.
//       return mapFromJson(json, propertyName, customPropertyFromJson, path);
...
//   // ! Throws exception on error.
//  Deserialize.propertiesFromJson(json, target, customPropertyFromJson, path);
...
//   // ! Throws exception on error.
//  this.plainObjectFromJson(mapJson, mapObject, customPropertyFromJson, path);
...
export class Serializable extends Attributable
{
  ...
  public static deserialize(data: string, path?: string): Serializable
  {
    ...
    // ! Throws exception on error.
    const className = getStringProperty(json, CLASS_NAME, path);
